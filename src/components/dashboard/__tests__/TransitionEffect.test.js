...
describe('TransactionEffects', () => {
  ...
  test('renders nothing when show is false', () => {
    const { container } = render(<TransactionEffects show={false} onFinish={mockOnFinish} />);
    expect(container.firstChild).toBeNull();
  });

  test('renders loading overlay when show is true', () => {
    const { getByText, getByTestId } = render(<TransactionEffects show={true} onFinish={mockOnFinish} />);
  ...
  test('does not call onFinish when show changes to false', () => {
    const { rerender } = render(<TransactionEffects show={true} onFinish={mockOnFinish} />);
  ...
  test('renders with correct classes and structure', () => {
    const { container } = render(<TransactionEffects show={true} onFinish={mockOnFinish} />);
    ...
    expect(contentDiv).toHaveClass('bg-white rounded-2xl p-8 flex flex-col items-center');
    expect(spinnerDiv).toHaveClass('w-16 h-16 border-4 border-blue-600 border-t-transparent rounded-full animate-spin mb-4');
