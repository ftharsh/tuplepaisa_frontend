...
describe("AuthPage Component", () => {
  ...
  test("toggles between sign-in and sign-up forms", () => {
    ...
    expect(
      screen.getByText(/Welcome back to your digital home/i)
    ).toBeInTheDocument();

    fireEvent.click(signUpButton);
    expect(
      screen.getByText(/Join our community and start your journey/i)
    ).toBeInTheDocument();
  ...
  test("submits registration form successfully", async () => {
    ...
    const signUpForm = screen
      .getByText(/Join our community and start your journey!/)
      .closest("form");
  ...
  test("handles error messages", async () => {
    ...
    const usernameInput = within(signInForm).getByPlaceholderText("Username");
    const passwordInput = within(signInForm).getByPlaceholderText("Password");
    const signInButton = within(signInForm).getByRole("button", {
      name: /Sign In/i,
    });
