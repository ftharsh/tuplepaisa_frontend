...
describe("walletService", () => {
  ...
  describe("rechargeWallet", () => {
    test("recharges wallet successfully", async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        "http://localhost:8080/api/wallet/recharge?amount=100",
        {
          method: "POST",
          headers: {
            Authorization: "Bearer mock-token",
    ...
    test("throws an error when response is not ok", async () => {
      ...
      await expect(walletService.rechargeWallet(100)).rejects.toThrow(
        "Not Found"
      );
    });
  ...
  describe("transferMoney", () => {
    test("transfers money successfully", async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        "http://localhost:8080/api/wallet/transfer?recipientUsername=recipientId&amount=100",
        {
          method: "POST",
          headers: {
            Authorization: "Bearer mock-token",
    ...
    test("throws an error when response is not ok", async () => {
      ...
      await expect(
        walletService.transferMoney("recipientId", 100)
